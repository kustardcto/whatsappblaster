   <footer>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

     <div class="container footer" >
       <div class="row g-4 text-center text-md-start">
               
                <div class="col-lg-3 col-md-6 "  >
                    <div class="d-flex align-items-center justify-content-center justify-content-md-start mb-3">
                        <div class="logo-placeholder me-2">
                            <i class="fab fa-whatsapp"></i>
                        </div>
                        <h1 style="color: white; font-size: 1.5rem; margin: 0;">WhatsApp Button</h1>
                        
                    </div>
                    <p style="font-size: 0.9rem; margin-bottom: 1rem; text-align: center;" class="text-center text-md-start">Your trusted partner for bulk messaging solutions. Reach your customers instantly with our reliable platform.</p>
                    <div class="social-links d-flex gap-3 justify-content-center justify-content-md-start">
                        <a href="#" class="text-white"><i class="fab fa-facebook-f"></i></a>
                        <a href="#" class="text-white"><i class="fab fa-twitter"></i></a>
                        <a href="#" class="text-white"><i class="fab fa-instagram"></i></a>
                        <a href="#" class="text-white"><i class="fab fa-linkedin-in"></i></a>
                        <a href="#" class="text-white"><i class="fab fa-whatsapp"></i></a>
                    </div>
                </div>

                <div class="col-lg-2 col-md-4 mx-auto">
                    <h5 class="footer-heading mb-3">Quick Links</h5>
                    <ul class="list-unstyled">
                        <li class="mb-2"><a href="{{route('home')}}" style="color: var(--footer-text); text-decoration: none; transition: all 0.3s; display: block;" onmouseover="this.style.color='var(--primary-color)'" onmouseout="this.style.color='var(--footer-text)'">Home</a></li>
                        <li class="mb-2"><a href="#key-features" style="color: var(--footer-text); text-decoration: none; transition: all 0.3s; display: block;" onmouseover="this.style.color='var(--primary-color)'" onmouseout="this.style.color='var(--footer-text)'">Features</a></li>
                        <!-- <li class="mb-2"><a href="#" style="color: var(--footer-text); text-decoration: none; transition: all 0.3s; display: block;" onmouseover="this.style.color='var(--primary-color)'" onmouseout="this.style.color='var(--footer-text)'">Database</a></li> -->
                        <li class="mb-2"><a href="{{route('virtual')}}" style="color: var(--footer-text); text-decoration: none; transition: all 0.3s; display: block;" onmouseover="this.style.color='var(--primary-color)'" onmouseout="this.style.color='var(--footer-text)'">SMS Pricing</a></li>
                        <li><a href="{{route('contactus')}}" style="color: var(--footer-text); text-decoration: none; transition: all 0.3s; display: block;" onmouseover="this.style.color='var(--primary-color)'" onmouseout="this.style.color='var(--footer-text)'">Contact</a></li>
                    </ul>
                </div>

                <div class="col-lg-2 col-md-4 mx-auto">
                    <h5 class="footer-heading mb-3">Our Services</h5>
                    <ul class="list-unstyled">
                        <li class="mb-2"><a href="#whatsapp-panel-features" style="color: var(--footer-text); text-decoration: none; transition: all 0.3s; display: block;" onmouseover="this.style.color='var(--primary-color)'" onmouseout="this.style.color='var(--footer-text)'">Bulk Messaging</a></li>
                        <li class="mb-2"><a href="{{route('virtual')}}" style="color: var(--footer-text); text-decoration: none; transition: all 0.3s; display: block;" onmouseover="this.style.color='var(--primary-color)'" onmouseout="this.style.color='var(--footer-text)'">WhatsApp Features</a></li>
                        <li><a href="#partnership" style="color: var(--footer-text); text-decoration: none; transition: all 0.3s; display: block;" onmouseover="this.style.color='var(--primary-color)'" onmouseout="this.style.color='var(--footer-text)'">Partnership</a></li>
                    </ul>
                </div>

                <div class="col-lg-3 col-md-6 mx-auto">
                    <h5 class="footer-heading mb-3">Contact Us</h5>
                    <ul class="list-unstyled">
                        <li class="mb-2 d-flex align-items-start justify-content-center justify-content-md-start">
                            <i class="fas fa-map-marker-alt mt-1 me-2" style="min-width: 16px;"></i>
                            <span>123 Business Street, City, Country</span>
                        </li>
                        <li class="mb-2 d-flex align-items-center justify-content-center justify-content-md-start">
                            <i class="fas fa-phone-alt me-2" style="min-width: 16px;"></i>
                            <span>+1 234 567 890</span>
                        </li>
                        <li class="d-flex align-items-center justify-content-center justify-content-md-start">
                            <i class="fas fa-envelope me-2" style="min-width: 16px;"></i>
                            <span>info@whatsappblastpro.com</span>
                        </li>
                    </ul>
                </div>
            </div>

            <div class="footer-bottom">
                <div class="row align-items-center">
                    <div class="col-md-6 text-center text-md-start mb-3 mb-md-0">
                        <p class="mb-0">&copy; 2023 WhatsApp Blast Pro. All rights reserved.</p>
                    </div>
                  
         <div class="col-md-6 d-flex align-items-center justify-content-center justify-content-md-end">
                <ul class="footer-bottom-links list-unstyled d-inline-flex mb-0 align-items-center"> 
                 <li class="me-3"><a href="#" class="footer-bottom-link">Privacy Policy</a></li>
                 <li class="me-3"><a href="#" class="footer-bottom-link">Terms of Service</a></li>
                 <li class="me-3"><a href="#" class="footer-bottom-link">Sitemap</a></li>
             </ul>
        </div>
                </div>
            </div>
        </div>
     </div>
           
    </footer>